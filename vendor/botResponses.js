...
const responses = async function (msg, bot, gCalendarObject) {
  //responses for tagged bot
  ...
  if (msg.content.includes(bot.user.id)) {
    ...
    questionsData.questions.forEach((item) => {
      manager.addDocument("es", item.text, item.category);
    });

    //Training answers
    answersData.answers.forEach((item) => {
      manager.addAnswer("es", item.category, item.text);
    });
